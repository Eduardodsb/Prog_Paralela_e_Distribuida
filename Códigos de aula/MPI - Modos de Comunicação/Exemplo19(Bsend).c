/*
Modo de comunicação: Bufferizado

Neste modo não existe a necessidade de haver uma operação de recepção, para que seja iniciado o envio.
*/
...
int     tam_buffer;
    ...
    /* Faz a alocação do espaço necessário para o buffer de envio */
    ...
    for (i=1; i <= (num_procs/2); i += i) {
        if ((meu_ranque/i)%2 == 0) 
            ...
            destino = meu_ranque-i;
    /* Realiza as operações de envio e recepção em qualquer ordem */
   ...
   /* Pode realizar a operação de redução agora com os dados recebidos */
    ...
    MPI_Buffer_detach(&buffer, &tam_buffer);  /*É necessário liberar o buffer com free*/
    free (buffer);
    MPI_Finalize();
    return 0;
}